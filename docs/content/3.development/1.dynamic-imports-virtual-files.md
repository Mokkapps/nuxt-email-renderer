...
Imports with `?inline` are also handled during this Nitro build step for supported image assets. Those files are read at build time and converted into `data:` URL exports before the email template is rendered.
...
- **Inline assets**: Local `.gif`, `.jpg`, `.jpeg`, `.png`, `.svg`, and `.webp` files can be imported with `?inline`
