...
To get started, add `nuxt-image-renderer` to your project:
...
export default defineNuxtConfig({
  ...
  nuxtEmailRenderer: {
    ...
    i18n: {
      enabled: true,
      defaultLocale: "en",
      localesDir: "./emails/locales", // Optional: directory with locale files
    },
  },
});
```
...
| `i18n`      | `object`  | See below   | i18n configuration for email templates     |
...
| `defaultLocale` | `string`  | `'en'`      | Default locale to use when no locale is specified  |
| `localesDir`    | `string`  | `undefined` | Directory containing locale message files          |

::callout{type="info"}
**Note:** To use i18n features, you need to install `vue-i18n`: `npm install vue-i18n`
::
