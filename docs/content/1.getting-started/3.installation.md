...
export default defineNuxtConfig({
  ...
  nuxtEmailRenderer: {
    ...
    // Enable/disable ECodeBlock + Shiki code highlighting (default: true)
    codeHighlighting: true,

    // CSS files (relative to project root) injected into every email template
...
| `codeHighlighting` | `boolean` | `true` | Enables `ECodeBlock` and Shiki highlighting. Set to `false` to exclude both from build output when unused. |
| `globalCss` | `string[]` | `[]` | Paths to CSS files (relative to project root) injected into every email as a `<style>` tag. Use to share CSS variables, resets, or fonts across all templates. |
