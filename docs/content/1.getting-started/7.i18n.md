...
When `vueI18n` is not used, locale messages can also be loaded from individual files via `locales[].file` (JSON, YAML, or YML), the same formats `@nuxtjs/i18n` supports.
